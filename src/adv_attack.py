...
def adv_whitebox(model, X, y, tau, eps, n_steps=200, step_size=0.2, data_range=(0, 1), loss_function="CE",
    ...
    specLB, specUB = large_box.concretize() # specified lower and upper bound for the large box
    ...
    with torch.enable_grad():
        for _ in range(restarts):
            ...
            for i in range(ODI_num_steps + n_steps+1):
                ...
                with torch.enable_grad():
                    out = model(X_pgd)
                    # found adversarial examples with wrong labels
    ...
    if tau == 0:
        ...
        if shift:
            # midpoints are clamped within [LB + tau, UB - tau] so that the box is within the large box region.
...
def adv_whitebox_L2(model, X, y, tau, eps, n_steps=200, step_size=0.2, data_range=(0, 1), loss_function="CE",
                 ODI_num_steps=10, ODI_step_size=1., restarts=1, train=True, rand_init=True, shift=True,
                 early_stopping=True,  y_target=None, dimwise_scaling=False):
    large_box = HybridZonotope.construct_from_noise(x=X, eps=eps, domain="box", data_range=data_range)
    specLB, specUB = large_box.concretize() # specified lower and upper bound for the large box
    ...
    with torch.enable_grad():
        for _ in range(restarts):
            ...
            # change to L2
            delta = torch.empty_like(X_pgd).normal_()
            d_flat = delta.view(X_pgd.size(0), -1)
            n = d_flat.norm(p=2, dim=1).view(X_pgd.size(0), 1, 1, 1)
            r = torch.zeros_like(n).uniform_(0, 1)
            delta *= r / n * eps
            X_pgd = torch.clamp(X_pgd + delta, min=specLB, max=specUB).detach()
            X_pgd = Variable(torch.clamp(X_pgd + delta, min=specLB, max=specUB), requires_grad=True)
            ...
            for i in range(ODI_num_steps + n_steps+1):
                ...
                if i < ODI_num_steps:
                    adv_images = X_pgd.detach() + ODI_step_size * lr_scale * grad
                else:
                    adv_images = X_pgd.detach() + lr_scheduler.get_lr() * lr_scale * grad
                    lr_scheduler.step()
