...
--epochs 180 --eps_end 0.14117647058 --eps_test 0.14117647058 --dataset tinyimagenet \
 --end_epoch_eps 82 --cert_reg bound_reg --lambda_ratio 0.00001 --l1 1e-6 --L2_attack"
...
python abcrown.py --config exp_configs/tinyimagenet_l2_2.yaml --complete_verifier skip --share_alphas --onnx_path $MODEL_PATH > out_l2_tiny_l2.txt
python abcrown.py --config exp_configs/tinyimagenet_l1_2.yaml --complete_verifier skip --share_alphas --onnx_path $MODEL_PATH > out_l1_tiny_l2.txt
python abcrown.py --config exp_configs/tinyimagenet_linf_2.yaml --complete_verifier skip --share_alphas --onnx_path $MODEL_PATH > out_linf_tiny_l2.txt

# Step 5: calculate union accuracy
python cal_union.py --dataset tiny --file_path l2
